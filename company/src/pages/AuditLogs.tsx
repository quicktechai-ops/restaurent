    ...
    <div className="p-6">
      ...
      <div className="card mb-6">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <select value={branchFilter || ''} onChange={(e) => { setBranchFilter(e.target.value ? parseInt(e.target.value) : undefined); setPage(1) }} className="input-field">
            <option value="">All Branches</option>
            {branches?.data?.map((b: any) => <option key={b.id} value={b.id}>{b.name}</option>)}
          </select>
          <select value={userFilter || ''} onChange={(e) => { setUserFilter(e.target.value ? parseInt(e.target.value) : undefined); setPage(1) }} className="input-field">
          ...
          <input type="date" value={fromDate} onChange={(e) => { setFromDate(e.target.value); setPage(1) }} className="input-field" placeholder="From Date" />
          <input type="date" value={toDate} onChange={(e) => { setToDate(e.target.value); setPage(1) }} className="input-field" placeholder="To Date" />
        </div>
      </div>
