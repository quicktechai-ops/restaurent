  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{t('nav.modifiers')}</h1>
        <button onClick={() => setShowForm(true)} className="btn-primary flex items-center gap-2"><Plus size={20} /> {t('modifiers.addModifier')}</button>
      </div>

      {showForm && (
        <div className="card p-6 mb-6">
          <h2 className="text-lg font-semibold mb-4">{editingId ? t('modifiers.editModifier') : t('modifiers.addModifier')}</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="label">{t('common.name')} *</label><input className="input" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required placeholder={t('modifiers.namePlaceholder')} /></div>
            ...
            <div><label className="label">{t('modifiers.extraPrice')}</label><input type="number" step="0.01" className="input" value={formData.extraPrice} onChange={e => setFormData({...formData, extraPrice: parseFloat(e.target.value) || 0})} /></div>
            ...
            <div className="md:col-span-2"><label className="label">{t('common.description')}</label><input className="input" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} /></div>
      ...
      <div className="card overflow-hidden">
        {isLoading ? <div className="p-8 text-center">{t('common.loading')}</div> : (
          <table className="table">
            <thead><tr><th>{t('common.name')}</th><th>{t('categories.arabic')}</th><th>{t('modifiers.extraPrice')}</th><th>{t('common.actions')}</th></tr></thead>
            <tbody>
              {modifiers?.data?.map((mod: any) => (
                <tr key={mod.id}>
                  ...
                  <td>{mod.nameAr || '-'}</td>
                  <td>{mod.extraPrice > 0 ? `${mod.currencyCode} ${mod.extraPrice.toFixed(2)}` : t('modifiers.free')}</td>
                  <td className="flex gap-2">
                    ...
                    <button onClick={() => confirm(t('common.confirmDelete')) && deleteMutation.mutate(mod.id)} className="text-red-600 hover:text-red-800"><Trash2 size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
