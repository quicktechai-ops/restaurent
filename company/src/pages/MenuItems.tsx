...
import { Plus, Edit, Trash2, UtensilsCrossed, Search, X } from 'lucide-react'
...
export default function MenuItems() {
  ...
  const [formData, setFormData] = useState({ 
    ...
    kitchenStationId: '', commissionPerUnit: 0
  })
  ...
  const resetForm = () => { 
    ...
    setFormData({ name: '', nameAr: '', code: '', description: '', categoryId: 0, defaultPrice: 0, currencyCode: 'USD', taxIncluded: false, allowSizes: false, sizes: [], modifierIds: [], kitchenStationId: '', commissionPerUnit: 0 }) 
  }
        ...
        <div className="card p-6 mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className="label">Category *</label>
              <select className="input" value={formData.categoryId} onChange={e => setFormData({...formData, categoryId: parseInt(e.target.value)})} required>
            ...
            <div><label className="label">Kitchen Station</label>
              <select className="input" value={formData.kitchenStationId} onChange={e => setFormData({...formData, kitchenStationId: e.target.value})}>
              ...
              <div className="md:col-span-2 border rounded-lg p-4 bg-gray-50">
                ...
                {formData.sizes.length > 0 && (
                  <table className="w-full text-sm">
                    <thead><tr className="border-b"><th className="text-left py-2">Size</th><th className="text-left py-2">Price</th><th className="text-left py-2">Cost</th><th></th></tr></thead>
                    <tbody>
              ...
              {items?.data?.map((item: any) => (
                <tr key={item.id}>
                  ...
                  <td className="flex gap-2">
                    <button onClick={() => { setEditingId(item.id); setFormData({ name: item.name, nameAr: item.nameAr || '', code: item.code || '', description: item.description || '', categoryId: item.categoryId, defaultPrice: item.defaultPrice, currencyCode: item.currencyCode, taxIncluded: item.taxIncluded, allowSizes: item.allowSizes, sizes: item.sizes || [], modifierIds: [], kitchenStationId: item.kitchenStationId?.toString() || '', commissionPerUnit: item.commissionPerUnit || 0 }); setShowForm(true) }} className="text-blue-600 hover:text-blue-800"><Edit size={18} /></button>
