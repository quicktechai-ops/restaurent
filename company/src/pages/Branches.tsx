  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{t('nav.branches')}</h1>
        <button onClick={() => setShowForm(true)} className="btn-primary flex items-center gap-2">
          <Plus size={20} /> {t('branches.addBranch')}
        </button>
      </div>

      {showForm && (
        <div className="card p-6 mb-6">
          <h2 className="text-lg font-semibold mb-4">{editingId ? t('branches.editBranch') : t('branches.addBranch')}</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="label">{t('branches.currency')}</label>
            ...
            <div>
              <label className="label">{t('branches.vatPercent')}</label>
            ...
            <div>
              <label className="label">{t('branches.serviceChargePercent')}</label>
      ...
      <div className="card overflow-hidden">
        {isLoading ? (
          ...
          <table className="table">
            <thead><tr><th>{t('common.name')}</th><th>{t('branches.city')}</th><th>{t('branches.currency')}</th><th>{t('pos.vat')}</th><th>{t('common.status')}</th><th>{t('common.actions')}</th></tr></thead>
            <tbody>
              {branches?.data?.map((branch: any) => (
                <tr key={branch.id}>
                  ...
                  <td className="flex gap-2">
                    ...
                    <button onClick={() => confirm(t('common.confirmDelete')) && deleteMutation.mutate(branch.id)} className="text-red-600 hover:text-red-800"><Trash2 size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
